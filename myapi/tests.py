...
class MyAPITest(TestCase):
    ...
    def setUpTestData(cls):
        cls.artist1 = Artist.objects.create(first_name="Jon", last_name="Favreau", birth_date="1966-10-19")
        cls.artist2 = Artist.objects.create(first_name="Robert", last_name="Downey", nick_name="Robert Downey Jr.", birth_date="1965-04-04")
    ...
    def test_get_movies_detail_by_name(self):
        response = self.client.get(reverse('movie-detail', args=[1])) # این طوری دیگه ارور نمیده و پاس میشه. چون تو دیتابیس تست تازه یه آرتیست ساختیم و یه فیلم که کارگردانش همون هست و جزییات فیلم ۱ رو میتونه به ما بده. اما اگه بزنیم فیلم ۲۰ بهمون ارور ۴۰۴ میده چون وجود نداره
        self.assertEqual(response.status_code, 200)
